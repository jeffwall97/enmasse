...
type AddressSpaceLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.AddressSpace, err error)
...
// List lists all AddressSpaces in the indexer.
func (s *addressSpaceLister) List(selector labels.Selector) (ret []*v1alpha1.AddressSpace, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.AddressSpace))
	})
	return ret, err
}
...
type AddressSpaceNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.AddressSpace, error)
...
// List lists all AddressSpaces in the indexer for a given namespace.
func (s addressSpaceNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.AddressSpace, err error) {
...
// Get retrieves the AddressSpace from the indexer for a given namespace and name.
func (s addressSpaceNamespaceLister) Get(name string) (*v1alpha1.AddressSpace, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("addressspace"), name)
	}
	return obj.(*v1alpha1.AddressSpace), nil
}
